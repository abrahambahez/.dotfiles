...
log_info "=== Dank Linux Post-Install Script Started ==="
...
if [[ -f "$DATA_DIR/dank-dnf-packages.txt" ]]; then
    install_dnf_packages "$DATA_DIR/dank-dnf-packages.txt"
fi

if [[ -f "$DATA_DIR/dank-copr-packages.txt" ]]; then
    install_copr_packages "$DATA_DIR/dank-copr-packages.txt"
fi

if [[ -f "$DATA_DIR/dank-flatpak-apps.txt" ]]; then
    setup_flathub
    install_flatpak_apps "$DATA_DIR/dank-flatpak-apps.txt"
fi
